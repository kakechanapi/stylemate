...
- **現状**：**公開ブロッカー消化済（規約・オンボーディング・PWA・計測）。VoC 直前。コーデ試着（自分姿）は管理者ベータ稼働中**
- **本番URL**：https://stylemate-alpha.vercel.app
- **最新コミット**：`07256eb`（オンボーディング「3着→即提案」）
- **次の候補**：試着品質の確認 / 通知（Push） / アフィリエイト購入導線 / VoC 開始
- **開発者**：kakechanapi（一人開発、Claude Code でペアプロ）
- **ペルソナ**：✅ 女性ファースト確定（2026-06-26）

### ⚠️ ユーザー側の残作業（2026-07-03 時点）
1. **Supabase で migration 0010・0011 を実行**（0010=試着の private バケット、0011=計測テーブル。未実行だと試着キャッシュと計測が動かない）
2. **Vercel ダッシュボードで Web Analytics を有効化**（Project → Analytics → Enable）
3. **Gemini API キーが課金 tier か確認**（無料枠キーだとユーザーデータが Google の学習に使われる）
4. **コーデ試着の品質確認**（マイページで全身写真登録 → 提案 → 「A案のトップスを自分の姿で見る」）

## 🆕 2026-07-03 セッションの大成果（コミット10本超）

### PM 視点の総点検 → 致命傷を全部潰した
- **オンボーディング**：初回ログイン→ 性別・体型 → 写真AIで3着 → 即3案提案（/?suggest=1 で自動開始）。コールドスタート対策の本丸
- **PWA化 + OGP**：manifest・アイコン・OG画像（暫定デザイン）。ホーム画面追加とLINE共有プレビューが成立
- **計測基盤**：app_events テーブル（migration 0011）+ 提案/決定/服登録/試着/オンボの全イベント + Vercel Analytics。D1/D7・提案→決定率・離脱着数が測れる
- **利用規約・プライバシーポリシー公開**（/terms /privacy）：試着写真の扱い・楽天アフィリエイト表記込み。ログイン画面に同意文言

### コーデ試着（自分姿）ベータ = 管理者限定で稼働中
- /api/coord-tryon（IDM-VTON 流用）：A案だけ先行生成（16円）→ B/C は希望時のみ（プログレッシブ）
- キャッシュ：同じ写真×同じ服は再生成しない（photoVersion + 服画像ハッシュがキー）
- 自分の全身写真は端末内 IndexedDB のみ（マイページで登録）。結果は private バケット + signed URL（migration 0010）
- ラベルは実態通り「トップス1点を合成」と明記（期待値コントロール）

### 学習の大幅強化
- 「これに決定」→ 採用服を LIKE 一括記録 / 不採用案の服を弱シグナル NOPE 記録（同日重複ガード付き）
- refreshStyleProfile はシグナル強度別に分析（スワイプ/確定=最強/明示嫌い/不採用=弱）

### セキュリティ・信頼性の総点検
- **無認証だった API 4本に認証追加**（ai-outfit / classify-image / products/search / style-feed）
- JST バグの直し漏れ修正（ホーム・カレンダーの「今日」判定）
- void 非同期を await 化（学習記録・コストログが Vercel 凍結で欠落していた）
- Gemini モデルをバージョン固定（-latest エイリアスの単価5倍ジャンプ回避）+ コスト定数を実態化
- iPhone で全身写真登録がカメラ強制になる capture 属性を除去

### 技術スタック監査（.secretary/research/2026-07-03-stack-audit.md）
- **収益化時の地雷2つ**：Vercel Hobby は非商用限定（→Pro $20/月）/ Open-Meteo も非商用限定
- Supabase Free は1週間非アクティブで停止・Storage 1GB 監視
- 現構成は VoC 段階では最適（固定費0円）。収益化ラインは月約7,000円
...
### 公開準備
- ✅ **利用規約・プライバシーポリシー**（/terms /privacy 公開済・2026-07-03）
- ✅ **オンボーディング画面**（3着→即提案の一本道・2026-07-03）
- ✅ **PWA・OGP・計測**（2026-07-03）
- ⏳ **アプリ名確定**（"StyleMate" は仮、友達と相談予定。アイコン・OG画像も暫定）
- ⏳ **Yahoo!ショッピング APP ID**：制限解除待ち（下記）
- ⏳ **通知（Push）**：PWA 化済なので次の一手。毎朝の再訪動線
...
11. **`supabase/migrations/0010_private_coord_tryon_bucket.sql`** （コーデ試着結果の private バケット + RLS）⏳ **未実行（2026-07-03 作成）**
12. **`supabase/migrations/0011_app_events.sql`** （計測イベントテーブル）⏳ **未実行（2026-07-03 作成）**

未実行があればユーザーに確認してから流す。0010/0011 未実行でもアプリは動く（試着キャッシュと計測が無効なだけ）。
